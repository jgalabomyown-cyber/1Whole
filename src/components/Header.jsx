...
export default function Header({ onSidebar }) {
  ...
  return <header><nav className="top-nav">
    ...
    <div className="nav-settings"><div className="study-audio"><button className="audio-menu-btn" onClick={() => { setAudioOpen((value) => !value); setTrackOpen(false); }}><Icon name="music-note-beamed" /> <span className="audio-label">Ambience ▾</span> <span className="audio-status">{audioState}</span></button>{audioOpen && <div className="audio-dropdown-menu show"><div className="audio-header">SELECT FREQUENCY</div>{['lofi', 'binaural', 'whitenoise'].map((name) => <button className="audio-track-item" key={name} onClick={() => playAudio(name)}>$ play {name === 'whitenoise' ? 'white-noise' : `${name}-beats`}</button>)}</div>}</div><div className="user-profile-block"><div className="profile-details"><span className="profile-alias">{userProfile?.alias || user?.displayName || 'Authenticated user'}</span><span className="profile-email">{user?.email}</span></div><div className="avatar-wrapper"><img src={user?.photoURL || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=80&h=80&fit=crop&crop=faces'} alt="Operator Avatar" className="profile-avatar" /></div><button className="logout-btn" type="button" onClick={handleLogout}>LOGOUT</button></div></div>
  </nav></header>;
}
